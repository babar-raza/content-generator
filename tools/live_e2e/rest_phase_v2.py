#!/usr/bin/env python3
"""REST API Phase Test V2 - Per-topic subprocess runner

Runs a single topic through REST API with output validation and
frontmatter normalization. Designed to run in isolated subprocess.
"""
import os
import sys
import json
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="REST Phase V2 - Per-topic")
    ...
    try:
        ...
        logger.info(f"POST /api/jobs for topic: {args.topic}")
        ...
        if resp.status_code not in [200, 201]:
            ...
            if not output_path:
                result["status"] = "FAIL"
                result["error"] = "No output_path in response"
            else:
        ...
        result["status"] = "FAIL"
        result["error"] = str(e)
        logger.error(f"REST exception: {e}", exc_info=True)
