...
def test_mesh_imports():
    ...
    try:
        from src.mesh import CapabilityRegistry
        print("✓ Mesh - CapabilityRegistry")
        results.append(True)
    except Exception as e:
        print(f"✗ Mesh - CapabilityRegistry: {e}")
        results.append(False)
    
    try:
        from src.mesh import AsyncRuntimeManager, AsyncRuntime
        print("✓ Mesh - Runtime (with alias)")
        results.append(True)
    except Exception as e:
        print(f"✗ Mesh - Runtime: {e}")
        results.append(False)
    
    try:
        from src.mesh import CacheManager, LRUCache
        print("✓ Mesh - Cache (with alias)")
        results.append(True)
    except Exception as e:
        print(f"✗ Mesh - Cache: {e}")
        results.append(False)
    
    try:
        from src.mesh import NegotiationManager, StateStore, MeshObserver
        print("✓ Mesh - Additional components")
        results.append(True)
    except Exception as e:
        print(f"✗ Mesh - Additional: {e}")
        results.append(False)
