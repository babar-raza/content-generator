...
async def list_agents():
    ...
    Requires executor to be initialized. If executor doesn't support get_agents,
    returns empty list.
...
async def list_all_agent_failures(
    limit: int = Query(default=50, le=200, description="Maximum number of failures to return")
):
    """List recent failures across all agents.
    ...
    try:
        ...
        try:
            ...
            # If monitor is unavailable, return empty list
            logger.warning(f"Health monitor unavailable: {monitor_error}")
            return {
                "failures": [],
                "total": 0,
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "warning": "Health monitor not available in mock mode"
            }

    except Exception as e:
        logger.error(f"Error listing all agent failures: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to list agent failures: {str(e)}")
...
async def get_agent(agent_id: str):
    ...
    """
    try:
        # Check if executor has get_agent method
        if _executor is None or not hasattr(_executor, 'get_agent'):
            raise HTTPException(status_code=501, detail="Agent lookup not supported by executor")
...
async def get_agent_failures(
    ...
    Raises:
        HTTPException 404: If agent is not found in the system
        HTTPException 501: If agent existence cannot be verified (monitor unavailable)
    """
    try:
        # Try to get health monitor
        try:
            ...
            # If monitor is unavailable (mock mode), we can't verify agent existence
            # Return 501 (Not Implemented) to indicate this feature requires the monitor
            raise HTTPException(
                status_code=501,
                detail="Agent failure tracking not available (health monitor not initialized)"
            )

        # Check if agent is known to the monitor (has execution history or failures)
        with monitor._lock:
            agent_has_history = (
                agent_id in monitor.execution_history or
                agent_id in monitor.recent_failures
            )

        # If agent has history, return its failures (or empty list if none)
        if agent_has_history:
            # Agent exists in monitor, proceed to get failures
            pass
        else:
            # Agent has no history in monitor
            # Try to verify if it's a known agent via executor
            if _executor is not None and hasattr(_executor, 'get_agents'):
                try:
                    agents = _executor.get_agents()
                    agent_in_executor = any(
                        a.get("id") == agent_id or a.get("name") == agent_id
                        for a in agents
                    )

                    if not agent_in_executor:
                        # Agent explicitly not in executor's list → doesn't exist
                        raise HTTPException(
                            status_code=404,
                            detail=f"Agent '{agent_id}' not found in the system"
                        )
                    # else: agent is in executor but has no failures yet, continue to return empty list
                except HTTPException:
                    # Re-raise HTTP exceptions (like our 404)
                    raise
                except Exception:
                    # If we can't check executor due to other errors, be lenient and return empty failures
                    # This allows testing agents that haven't executed yet
                    pass
            # else: executor not available, be lenient and return empty failures

        # Agent exists, get its failures
        failures_data = monitor.get_agent_failures(agent_id, limit=limit)
