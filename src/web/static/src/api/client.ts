...
class APIClient {
  ...
  async getAgentJobs(agentId: string, limit: number = 50, status?: string): Promise<any> {
    let url = `/agents/${agentId}/jobs?limit=${limit}`;
    if (status) url += `&status=${status}`;
    const response = await this.apiClient.get(url);
    return response.data;
  }
  ...
  // ============================================================================
  // NEW: Enhanced Visualization API (Task Card 04)
  // ============================================================================

  async getWorkflowsViz(format: string = 'json'): Promise<any> {
    const response = await this.apiClient.get(`/viz/workflows?format=${format}`);
    return response.data;
  }

  async getWorkflowGraph(workflowId: string, jobId?: string): Promise<any> {
    let url = `/viz/graph/${workflowId}`;
    if (jobId) url += `?job_id=${jobId}`;
    const response = await this.apiClient.get(url);
    return response.data;
  }

  async getVizMetrics(workflowId?: string, timeRange: string = '24h'): Promise<any> {
    let url = `/viz/metrics?time_range=${timeRange}`;
    if (workflowId) url += `&workflow_id=${workflowId}`;
    const response = await this.apiClient.get(url);
    return response.data;
  }

  async getAgentsViz(): Promise<any> {
    const response = await this.apiClient.get('/viz/agents');
    return response.data;
  }

  async getFlowsViz(workflowId?: string, jobId?: string): Promise<any> {
    let url = '/viz/flows?';
  ...
  async analyzeBottlenecks(workflowId?: string, thresholdSeconds: number = 5.0): Promise<any> {
    let url = `/viz/bottlenecks?threshold_seconds=${thresholdSeconds}`;
    if (workflowId) url += `&workflow_id=${workflowId}`;
    const response = await this.apiClient.get(url);
    return response.data;
  }

  async getDebugViz(jobId: string): Promise<any> {
    const response = await this.apiClient.get(`/viz/debug/${jobId}`);
    return response.data;
  }

  async discoverTopicsAPI(params: {
    kb_path?: string;
    docs_path?: string;
    content?: string;
    max_topics?: number;
    ...
    const response = await this.apiClient.post('/topics/discover', params);
    return response.data;
  }

  async listTopicsAPI(): Promise<any> {
    const response = await this.apiClient.get('/topics/list');
    return response.data;
  }

  async getTopicDetails(topicId: string): Promise<any> {
    const response = await this.apiClient.get(`/topics/${topicId}`);
    return response.data;
  }

  async ingestKB(path: string): Promise<any> {
    const response = await this.apiClient.post('/ingest/kb', { path });
    return response.data;
  }

  async ingestDocs(path: string): Promise<any> {
    const response = await this.apiClient.post('/ingest/docs', { path });
    return response.data;
  }

  async ingestAPI(path: string): Promise<any> {
    const response = await this.apiClient.post('/ingest/api', { path });
    return response.data;
  }

  async ingestBlog(path: string): Promise<any> {
    const response = await this.apiClient.post('/ingest/blog', { path });
    return response.data;
  }

  async ingestTutorial(path: string): Promise<any> {
    const response = await this.apiClient.post('/ingest/tutorial', { path });
    return response.data;
  }

  async getIngestionStatus(): Promise<any> {
    const response = await this.apiClient.get('/ingest/status');
    return response.data;
  }

  async getToneConfig(): Promise<any> {
    const response = await this.apiClient.get('/config/tone');
    return response.data;
  }

  async getPerformanceConfig(): Promise<any> {
    const response = await this.apiClient.get('/config/performance');
    return response.data;
  }

  async triggerHotReload(): Promise<any> {
    const response = await this.apiClient.post('/config/hot-reload');
    return response.data;
  }

  async getSystemDiagnostics(): Promise<any> {
    const response = await this.apiClient.get('/debug/system');
    return response.data;
  }

  async debugAgent(agentId: string): Promise<any> {
    const response = await this.apiClient.get(`/debug/agent/${agentId}`);
    return response.data;
  }

  async debugJob(jobId: string): Promise<any> {
    const response = await this.apiClient.get(`/debug/job/${jobId}`);
    return response.data;
  }

  async getPerformanceProfile(): Promise<any> {
    const response = await this.apiClient.get('/debug/performance');
    return response.data;
  }
}
