"""Tutorial Search Agent - Searches tutorial content for relevant context."""
...
class TutorialSearchAgent(SelfCorrectingAgent, Agent):

    """Searches tutorial content for relevant context."""

    def __init__(self, config: Config, event_bus: EventBus, database_service: DatabaseService):

        self.database_service = database_service

        Agent.__init__(self, "TutorialSearchAgent", config, event_bus)

    def _create_contract(self) -> AgentContract:

        return AgentContract(

            agent_id="TutorialSearchAgent",

            capabilities=["gather_rag_tutorial"],
    ...
    def _subscribe_to_events(self):
        ...
        self.event_bus.subscribe("execute_gather_rag_tutorial", self.execute)
    ...
    def execute(self, event: AgentEvent) -> Optional[AgentEvent]:
        ...
        logger.info(f"Found {len(context)} tutorial context chunks")

        return AgentEvent(

            event_type="rag_complete",

            data={"source": "tutorial", "context": context},
