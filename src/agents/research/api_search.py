"""API Search Agent - Searches API documentation for relevant content."""
...
class APISearchAgent(SelfCorrectingAgent, Agent):

    """Searches API docs for code generation context."""

    def __init__(self, config: Config, event_bus: EventBus, database_service: DatabaseService):

        self.database_service = database_service

        Agent.__init__(self, "APISearchAgent", config, event_bus)

    def _create_contract(self) -> AgentContract:

        return AgentContract(

            agent_id="APISearchAgent",

            capabilities=["gather_rag_api"],
    ...
    def _subscribe_to_events(self):
        ...
        self.event_bus.subscribe("execute_gather_rag_api", self.execute)
    ...
    def execute(self, event: AgentEvent) -> Optional[AgentEvent]:
        ...
        logger.info(f"Found {len(context)} API context chunks")
