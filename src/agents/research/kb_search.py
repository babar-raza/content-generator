"""KB Search Agent - Searches knowledge base for relevant content."""
...
class KBSearchAgent(SelfCorrectingAgent, Agent):
    ...
    def __init__(self, config: Config, event_bus: EventBus, database_service: DatabaseService):
        ...
        Agent.__init__(self, "KBSearchAgent", config, event_bus)

    def _create_contract(self) -> AgentContract:

        return AgentContract(

            agent_id="KBSearchAgent",

            capabilities=["gather_rag_kb"],
    ...
    def _subscribe_to_events(self):
        ...
        self.event_bus.subscribe("execute_gather_rag_kb", self.execute)
    ...
    def execute(self, event: AgentEvent) -> Optional[AgentEvent]:
        ...
        logger.info(f"Found {len(context)} KB context chunks")
