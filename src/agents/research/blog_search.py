"""Blog Search Agent - Searches blog posts for relevant content."""
...
class BlogSearchAgent(SelfCorrectingAgent, Agent):

    """Searches blog posts for relevant context."""

    def __init__(self, config: Config, event_bus: EventBus, database_service: DatabaseService):

        self.database_service = database_service

        Agent.__init__(self, "BlogSearchAgent", config, event_bus)

    def _create_contract(self) -> AgentContract:

        return AgentContract(

            agent_id="BlogSearchAgent",

            capabilities=["gather_rag_blog"],
    ...
    def _subscribe_to_events(self):
        ...
        self.event_bus.subscribe("execute_gather_rag_blog", self.execute)
    ...
    def execute(self, event: AgentEvent) -> Optional[AgentEvent]:
        ...
        logger.info(f"Found {len(context)} blog context chunks")
